...
namespace dOSCEngine.Engine.Nodes.Connector.VRChat
{
    ...
    {
        public OSCVRCAvatarFloatReadNode(string SelectedOption = OSCService.VelocityZ, OSCService? service = null, Point? position = null) : base(position ?? new Point(0, 0))
        {
        ...
        public OSCVRCAvatarFloatReadNode(Guid guid, string SelectedOption = OSCService.VelocityZ, OSCService? service = null, Point? position = null) : base(guid, position ?? new Point(0, 0))
        {
            AddPort(new NumericPort(PortGuids.PortGuidGenerator(1), this, false));
            _service = service;
            _service.OnOSCMessageRecieved += OnMessageReceived;
