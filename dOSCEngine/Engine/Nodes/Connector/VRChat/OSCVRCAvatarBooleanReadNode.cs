...
namespace dOSCEngine.Engine.Nodes.Connector.VRChat
{
    ...
    {
        public OSCVRCAvatarBooleanReadNode(string SelectedOption = OSCService.InStation, OSCService? service = null, Point? position = null) : base(position ?? new Point(0, 0))
        {
        ...
        public OSCVRCAvatarBooleanReadNode(Guid guid, string SelectedOption = OSCService.InStation, OSCService? service = null, Point? position = null) : base(guid, position ?? new Point(0, 0))
        {
            AddPort(new LogicPort(PortGuids.Port_1, this, false));
            _service = service;
            _service.OnOSCMessageRecieved += OnMessageReceived;
