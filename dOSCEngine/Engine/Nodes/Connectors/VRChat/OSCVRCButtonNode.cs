...
namespace dOSCEngine.Engine.Nodes.Connectors.VRChat
{
    ...
    {
        public OSCVRCButtonNode(string SelectedOption = OSCService.MoveForward, ServiceBundle? service = null, Point? position = null) : base(position ?? new Point(0, 0))
        {
            AddPort(new LogicPort(PortGuids.Port_1, this, true));
            _service = service?.OSC;
            this.SelectedOption = SelectedOption;
        }
        public OSCVRCButtonNode(Guid guid, string SelectedOption = OSCService.MoveForward, ServiceBundle? service = null, Point? position = null) : base(guid, position ?? new Point(0, 0))
        {
            AddPort(new LogicPort(PortGuids.Port_1, this, true));
            _service = service?.OSC;
            this.SelectedOption = SelectedOption;
