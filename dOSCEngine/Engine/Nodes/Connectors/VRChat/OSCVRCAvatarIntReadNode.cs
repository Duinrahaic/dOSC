...
namespace dOSCEngine.Engine.Nodes.Connectors.VRChat
{
    ...
    {
        public OSCVRCAvatarIntReadNode(string SelectedOption = OSCService.Face, ServiceBundle? service = null, Point? position = null) : base(position ?? new Point(0, 0))
        {
        ...
        public OSCVRCAvatarIntReadNode(Guid guid, string SelectedOption = OSCService.Face, ServiceBundle? service = null, Point? position = null) : base(guid, position ?? new Point(0, 0))
        {
            AddPort(new NumericPort(PortGuids.PortGuidGenerator(1), this, false));
            _service = service?.OSC;
            _service.OnOSCMessageRecieved += OnMessageReceived;
