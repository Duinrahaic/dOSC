...
{
    private readonly ConcurrentDictionary<(T data, string source), bool> committedSources = new(TupleComparer.Default);
    private readonly ConcurrentQueue<(T data, string source)> queue = new();
    ...
    public bool RemoveCommit(string source)
    {
        lock (queue)
        {
            ...
            if (committedSources.TryRemove(dummyTuple.Key, out _))
            {
                ...
                foreach (var itemToRemove in itemsToRemove) queue.TryDequeue(out _);
    ...
    {
        public static readonly TupleComparer Default = new();
