...
{
    public MultiPort(Guid guid, BaseNode parent, bool input, string name, bool limitLink = true,
        List<PortType>? allowedTypes = null) : base(guid, parent, input, name, limitLink, PortType.Multi)
    {
        if (allowedTypes == null) allowedTypes = Enum.GetValues(typeof(PortType)).Cast<PortType>().ToList();
        AllowedTypes = allowedTypes;
        OnPortLinksChanged += OnLinksChanged;
    }

    public List<PortType> AllowedTypes { get; }
    ...
    private void OnLinksChanged(Model obj)
    {
        ...
        {
            var newPortType = GetPortTypeOverride();
