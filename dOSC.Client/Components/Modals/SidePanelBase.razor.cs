...
public partial class SidePanelBase
{
    ...
    [Inject] private dOSCService? Engine { get; set; }
    ...
    public string CurrentAppIcon
    {
        get
        {
            if (App.Data == null) return AppDefaults.GetDefaultAppImage();
    ...
    [Parameter] public EventCallback<AppLogic> OnUpdate { get; set; }
    ...
    {
        if (App != null) await JS.DownloadApp(App.GetDTO());
    }
}