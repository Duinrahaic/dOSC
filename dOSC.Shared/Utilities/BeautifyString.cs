    ...
    public static class BeautifyString
    {
        public static string BeautifyMilliseconds(TimeSpan timeSpan, bool NumbersOnly = false)
        {
            ...
            if (NumbersOnly) // Numbers Only
            {
                ...
                if (timeSpan.Days > 0)
                {
                    result = $"{(long)timeSpan.TotalDays:0}:{timeSpan.Hours:00}:{timeSpan.Minutes:00}:{timeSpan.Seconds:00}";
                }
            ...
            else // Text
            {
                if (timeSpan.Days > 0)
                {
                    result = $"{(long)timeSpan.TotalDays:0}d {timeSpan.Hours:00}h {timeSpan.Minutes:00}m {timeSpan.Seconds:00}s";
                }
                else if (timeSpan.Hours > 0)
                {
